...
export function DocumentDetail({ doc: initialDoc }: { doc: Document & { id: string; pathname?: string; mimeType?: string; assetPath?: string } }) {
  ...
  const [renameValue, setRenameValue] = useState(initialDoc.name)
  const [savingName, setSavingName] = useState(false)
  const [renameError, setRenameError] = useState('')

  const isManaged = Boolean(doc.pathname)
  ...
  async function handleRename() {
    ...
    if (!name || name === doc.name) { setRenaming(false); setRenameValue(doc.name); return }
    setSavingName(true)
    setRenameError('')
    try {
      ...
      setDoc((current) => ({ ...current, name: data.name }))
      setRenaming(false)
    } catch (err) {
      ...
      <div className="flex flex-col gap-4 rounded-3xl border border-border bg-card p-6 shadow-soft sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            {renaming ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter') handleRename(); if (e.key === 'Escape') { setRenaming(false); setRenameValue(doc.name) } }}
                  className="h-9 w-56 rounded-lg border border-border bg-background px-2.5 text-sm font-semibold text-foreground outline-none focus:ring-2 focus:ring-ring/60"
                />
                <button type="button" disabled={savingName} onClick={handleRename} className="rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground disabled:opacity-60">
                  {savingName ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : 'Save'}
                </button>
                <button type="button" onClick={() => { setRenaming(false); setRenameValue(doc.name); setRenameError('') }} className="rounded-lg px-3 py-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground">
                  Cancel
                </button>
              </div>
            ) : (
              <div className="flex items-center gap-2">
                ...
                {isManaged ? (
                  <button type="button" onClick={() => setRenaming(true)} className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:bg-secondary hover:text-foreground" title="Rename">
            ...
            {renameError ? <p role="alert" className="mt-1 text-xs font-medium text-destructive">{renameError}</p> : null}
      ...
      {confirmOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            {deleteError ? <p role="alert" className="mt-3 text-sm font-medium text-destructive">{deleteError}</p> : null}
            <div className="mt-6 flex items-center justify-end gap-3">
              ...
              <button
                disabled={deleting}
                onClick={isManaged ? handleDelete : () => router.push('/knowledge-base')}
                className="inline-flex items-center gap-2 rounded-xl bg-destructive px-5 py-2.5 text-sm font-semibold text-white shadow-soft transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lift disabled:cursor-not-allowed disabled:opacity-60"
              >
